...
if isfile(student_records_file)
    ...
    student_data = table(string([]), string([]), 'VariableNames', {'Name', 'ID'});
...
if isfile(attendanceFile)
    ...
    if ~isequal(attendance_data.Properties.VariableNames, expectedVars)
        ...
        attendance_data = table(string([]), string([]), 'VariableNames', expectedVars);
        writetable(attendance_data, attendanceFile);
    end
else
    attendance_data = table(string([]), string([]), 'VariableNames', {'Name', 'ID'});
    writetable(attendance_data, attendanceFile);
end
        ...
        for i = 1:size(bbox,1)
            ...
            if ~strcmp(bestMatch, 'Unknown')
                ...
                if ~ismember(studentID, seen_students)
                    newEntry = table(string(student_name), string(studentID), 'VariableNames', {'Name', 'ID'});

                    % Ensure table structure matches before concatenation
                    attendance_data = [attendance_data; newEntry]; %#ok<AGROW>
